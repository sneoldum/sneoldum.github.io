...
var game = {
  ...
  levelPassed: function() {
    ...
    levelPassedDescreption.innerHTML = 'You catched all the ' + fruitTapsCount + ' fruits!';
    nextLevelDesc.innerHTML = 'Now try to catch ' + gameCurrentLevel.goal + ' fruits.';
  },
  levelLost: function() {
    timesUpCont.style.display = "block";
    timesUpDescription.innerHTML = 'You catched ' + fruitTapsCount + ' fruits, and you had to catch ' + gameCurrentLevel.goal + ' at least to complete this level.';
  },
  timesUp: function() {
    timeEngines.stop();
    game.checkScore();
...
levelGoalTitle.innerHTML = 'Try to catch ' + gameCurrentLevel.goal + ' fruits in ' + gameCurrentLevel.time + ' seconds.';
// ------------------------------------------------------- //
// -------------------- fruit Object -------------------- //
var fruit = {
  tapCount: 0,
  names: ['101','102','103','104','105','106', '107', '108', '109', '110', '111', '112', '113', '114', '115', '116'],
  create: function(){
    ...
    if (document.getElementById('emoticon')) {
      ...
      fruitDiv.setAttribute('src', 'emoticons/' + fruit.names[fruit.random()] +'.png');
      ...
      fruitDiv.onclick = function() { // when tappign the emoticon
        fruit.tap();
      }
    }
  },
  ...
  randomPosition: function(){ // showing the emoticon box randomly
    fruitDiv.style.left = generateRandomNumb(fruitDivWidth, gameSpace.offsetWidth - fruitDivWidth) + 'px'; // random left position minus the emoticon width
    fruitDiv.style.top = generateRandomNumb(fruitDivWidth, gameSpace.offsetHeight - fruitDivHeight) + 'px'; // random top position minus the emoticon height
  },
  tap: function() {
...
pauseGameButton.onclick = function() {
  ...
  gamePausedDescription.innerHTML = 'You catched ' + fruitTapsCount + ' fruits, and you have to catch ' + gameCurrentLevel.goal + ' at least to complete this level.'
}
